...
function getProgressBar(rawValue) {
  ...
  if (tier == 0) {
    ...
    process =
      (rawValue - Difficulty.tierThreshold[tier - 1]) /
      (Difficulty.tierThreshold[tier] - Difficulty.tierThreshold[tier - 1]);
  } else {
    process = 1;
  }
...
function UpdateRank(player, value, source) {
  ...
  if (player.persistentData.getBoolean('disableRankChange')) return;
  value =
    Difficulty.getPlayerRawValue(player) + value < 0
      ? -Difficulty.getPlayerRawValue(player)
      : value;

  let oldTier = Difficulty.getPlayerTier(player);
  Difficulty.setPlayerRawValue(
    player,
    parseInt(Difficulty.getPlayerRawValue(player)) + parseInt(value)
  );
  ...
  if (value > 0) {
    if (newTier > oldTier) {
      player.server.runCommandSilent(
        `title ${player.username} title {"translate":"${tierKey}","color":"gold","bold":true}`
      );
      player.server.runCommandSilent(
        `title ${player.username} subtitle {"translate":"difficulty.createdelightcore.tier_up_subtitle","color":"gray"}`
      );
    ...
    let msg = Text.of('§a▲ ')
      .append(Text.translatable(tierKey))
      .append(Text.of(` §8[${bar}§8]`));
    player.displayClientMessage(msg, true);
  } else {
    if (source == 'death') {
      let msg = Text.translatable('difficulty.createdelightcore.death_penalty')
        .append(Text.of(' §7→ '))
        .append(Text.translatable(tierKey))
        .append(Text.of(` §8[${bar}§8]`));
      player.displayClientMessage(msg, true);
      if (newTier < oldTier) {
        player.server.runCommandSilent(
          `title ${player.username} subtitle {"translate":"difficulty.createdelightcore.tier_down_subtitle","color":"dark_red"}`
        );
...
FTBQuestsEvents.customReward((e) => {
  ...
  reward.tags.forEach((s) => {
    ...
    if (start == 'rank') {
      ...
      if (disableRankChange == null) player.persistentData.putBoolean('disableRankChange', true);
      else player.persistentData.putBoolean('disableRankChange', !disableRankChange);
      if (!disableRankChange) player.tell('已关闭难度变化！');
      else player.tell('已开启难度变化！');
    }
    if (mid != 'unrepeatable') {
      // Client.tell(`${player.uuid}, ${reward.quest}`)
      server.runCommand(
        `/execute as ${player.uuid} run ftbquests change_progress @s reset ${reward.quest}`
      );
    }
  });
});
