...
EntityEvents.drops((e) => {
  ...
  if (entity.isPlayer() || source.player == null) return;
  ...
  drops.forEach((itemEntity) => {
    ...
    if (guaranteedCount > 0) e.addDrop(item.copyWithCount(guaranteedCount));
    if (fractionalChance > 0) e.addDrop(item.copyWithCount(1), fractionalChance);
  });
});
