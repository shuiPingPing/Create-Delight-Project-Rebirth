...
Ponder.registry((event) => {
  // 序列装配
  event.create('create:precision_mechanism').scene(
    'createdelightcore:sequenced_assembly',
    '序列装配',
    ...
    (builder, util) => {
      ...
      scene.text(40, '按照配方搭建序列装配装置（啊好烦Jason自己完善说明吧）', [2, 3, 1]);
      scene.idle(40);
      ...
      // 创建在传送带上的金板
      let stall1 = PonderUtil.createItemOnBelt(
        scene,
        [4, 1, 1],
        Direction.down,
        'create:golden_sheet'
      ...
      PonderUtil.moveDeployer(scene, [3, 3, 1], -1, 10);
      PonderUtil.changeBeltItemTo(
        scene,
        stall1,
        new Item.of('create:incomplete_precision_mechanism')
      );
      ...
      let stall2 = PonderUtil.createItemOnBelt(
        scene,
        [3, 1, 1],
        Direction.up,
