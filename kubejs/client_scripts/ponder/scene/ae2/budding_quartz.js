Ponder.registry((event) => {
  event
    ...
    .scene(
      'createdelightcore:ae_budding_quartz_grow',
      '赛特斯石英母岩的生长',
      ...
      (builder, util) => {
        ...
        scene
          .text(100, '在生长过程中，赛特斯石英母岩有概率降低一个等级（除了无瑕母岩）')
          .attachKeyFrame();
        ...
        pos_list.forEach((pos) => {
          scene.world.setBlocks(
            [2 + pos[0], 1 + pos[1], 2 + pos[2]],
            'ae2:growth_accelerator',
            false
          );
          scene.world.setBlocks(
            [2 + pos[0], 2 + pos[1], 2 + pos[2]],
            'ae2:crystal_resonance_generator',
            false
          );
          scene.world.modifyBlock(
            [2 + pos[0], 1 + pos[1], 2 + pos[2]],
            (state) => state.with('facing', 'up'),
            false
          );
          scene.world.modifyBlock(
            [2 + pos[0], 2 + pos[1], 2 + pos[2]],
    ...
    .scene(
      'createdelightcore:ae_budding_quartz_repair',
      '赛特斯石英母岩的修复',
      ...
      (builder, util) => {
        ...
        scene.addKeyframe();
        scene.idle(20);
        scene.showControls(20, [2.5, 1.5, 2.5], 'down').rightClick().withItem('ae2:quartz_block');
        scene.idle(20);
        ...
        for (let index = budding_quartz_list.length - 2; index > 1; index--) {
          ...
          scene
            .showControls(20, [2.5, 1.5, 2.5], 'down')
            .rightClick()
