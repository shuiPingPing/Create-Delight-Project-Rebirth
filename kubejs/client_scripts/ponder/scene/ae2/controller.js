Ponder.registry((event) => {
  event
    ...
    .scene(
      'createdelightcore:me_controller',
      'ME控制器的摆放方式……',
      'createdelightcore:ponder_me_controller',
      (builder, util) => {
        let scene = new CDClientJavaClasses.$CreateSceneBuilder(builder);
        // [ponder-camera] 按结构尺寸适配（9x18x9，参考 Create 的 configureBasePlate/scaleSceneView/setSceneOffsetY 用法）
        ...
        scene.idle(60);
        scene
          .text(60, '控制器需要接入能源才能工作，使用其他mod的能源也可', [0.5, 1.5, 1.5])
          .attachKeyFrame();
        ...
        scene.world.modifyBlocks(
          [8, 1, 1, 1, 1, 1],
          (state) => state.with('state', 'conflicted'),
          false
        );
        scene.world.modifyBlocks(
          [1, 8, 1, 1, 1, 1],
