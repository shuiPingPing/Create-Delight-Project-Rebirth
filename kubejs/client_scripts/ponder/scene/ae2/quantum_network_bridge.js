Ponder.registry((e) => {
  e.create(['ae2:quantum_ring', 'ae2:quantum_link']).scene(
    'createdelightcore:quantum_network_bridge',
    '使用量子网桥将网络传递到远端',
    'createdelightcore:ponder_quantum_network_bridge',
    ...
    (builder, util) => {
      ...
      scene
        .text(60, '连接量子环和网络只能通过围绕量子连接仓的四个量子环方块', [1.5, 2.5, 1])
        .attachKeyFrame();
      scene.idle(80);
      for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
      ...
      scene
        .text(60, '量子环是成双成对使用的，你需要在你需要的远端也放置一个量子环结构')
        .attachKeyFrame();
      scene.idle(80);
