Ponder.registry((event) => {
  event
    .create(['ae2:interface', 'ae2:cable_interface'])
    .scene(
      'createdelightcore:interface',
      'ME接口的使用',
      'createdelightcore:ponder_ae_interface',
      (builder, util) => {
        ...
        (scene.idle(60), scene.world.showSection([3, 2, 2], Direction.DOWN));
        scene.addKeyframe();
        scene.idle(20);
        ...
        scene.idle(40);
        scene.showControls(40, [3.5, 2, 2.5], 'down').withItem('ae2:certus_quartz_crystal');
        scene.idle(40);
        scene.world.showSection([3, 1, 1], Direction.SOUTH);
        scene.idle(20);
        scene.text(60, '它会将自身填充物料至你所设定的值');
        scene.world.flapFunnel([3, 1, 1], true);
        scene.world.createItemEntity([3.5, 1.25, 1.5], [0, 0, 0], 'ae2:certus_quartz_crystal');
        scene.idle(60);
      }
    );
});
