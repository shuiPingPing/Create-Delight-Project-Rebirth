Ponder.registry((event) => {
  event
    .create(['ae2:pattern_provider', 'ae2:cable_pattern_provider'])
    .scene(
      'createdelightcore:pattern_provider',
      '样板供应器的使用',
      ...
      (builder, util) => {
        ...
        scene.idle(60);
        scene.showControls(20, [2.5, 3, 3.5], 'down').rightClick().withItem('create:wrench');
        scene.idle(20);
        scene.world.modifyBlock([2, 2, 3], (state) => state.with('push_direction', 'down'), false);
        scene.idle(20);
        scene
          .text(40, '使用扳手点击样板供应器可使其方向变为扳手点击的方向', [2.5, 3, 3.5])
          .attachKeyFrame();
        scene.idle(60);
        ...
        scene.idle(20);
        scene.world.removeItemsFromBelt([4, 1, 3]);
        scene.world.createItemEntity([5, 2, 3.5], [0.2, 0.25, 0], 'create:iron_sheet');
        scene.idle(20);
    ...
    .scene(
      'createdelightcore:ae_crafting_parallel',
      '自动合成的并行',
      ...
      (builder, util) => {
        ...
        scene.idle(20);
        scene.showControls(40, [1.5, 2, 3.5], 'down').whileSneaking().withItem('ae2:memory_card');
        scene.idle(60);
        ...
        scene.idle(40);
        scene.showControls(40, [2.5, 2, 3.5], 'down').withItem('ae2:memory_card');
        scene.idle(20);
        scene.world.modifyBlock([2, 1, 3], (bs) => bs.with('push_direction', 'south'), false);
        scene.idle(40);
        scene
          .text(40, '内存卡会消耗物品栏中的空白样板，并依照其记录的样板复制到目标样板供应器中')
          .attachKeyFrame();
        scene.idle(60);
  ...
  event
    ...
    .scene(
      'createdelightcore:pattern_provider_interaction',
      '样板供应器与ME接口的联动',
      ...
      (builder, util) => {
        ...
        scene.idle(60);
        scene.showControls(40, [1.5, 1.5, 1.5], 'down').withItem('ae2:certus_quartz_crystal');
        scene.idle(40);
        scene.world.createItemEntity([4.5, 1.5, 1.5], [0, 0, 0], 'ae2:certus_quartz_crystal');
        scene.idle(20);
