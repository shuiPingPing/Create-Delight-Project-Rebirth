Ponder.registry((e) => {
  e.create('ae2:formation_plane').scene(
    'createdelightcore:formation_plane',
    '成型面板的使用',
    'createdelightcore:ponder_formation_plane',
    (builder, util) => {
      ...
      scene.idle(80);
      let item1 = scene.world.createItemEntity([3.5, 5, 1.5], [0, 0, 0], 'ae2:quartz_block');
      scene.idle(15);
      ...
      scene.idle(80);
      item1 = scene.world.createItemEntity([3.5, 5, 1.5], [0, 0, 0], 'ae2:quartz_block');
      scene.idle(15);
      scene.world.modifyEntity(item1, (e) => {
        e.kill();
      });
      scene.world.createItemEntity([2.5, 1.5, 1.75], [0, 0, 0], 'ae2:quartz_block');
      scene.overlay.showOutline('green', {}, [2, 1, 1], 60);
      scene.text(60, '成型面板会将物品以掉落物形式丢弃在它的前面', [2.5, 1.5, 1.5]);
      scene.idle(80);
      scene.text(60, '配置成型面板的过滤选项可使其只输出过滤内的物品').attachKeyFrame();
      scene.idle(80);
      scene.showControls(40, [2.5, 1.5, 1.75], 'right').withItem('ae2:certus_quartz_crystal');
      scene.idle(20);
      item1 = scene.world.createItemEntity([3.5, 5, 1.5], [0, 0, 0], 'ae2:quartz_block');
      scene.idle(15);
      scene.world.modifyEntity(item1, (e) => {
      ...
      scene.idle(40);
      item1 = scene.world.createItemEntity([3.5, 5, 1.5], [0, 0, 0], 'ae2:certus_quartz_crystal');
      scene.idle(15);
      scene.world.modifyEntity(item1, (e) => {
        e.kill();
      });
      scene.world.createItemEntity([2.5, 1.5, 1.75], [0, 0, 0], 'ae2:certus_quartz_crystal');
    }
  );
});
