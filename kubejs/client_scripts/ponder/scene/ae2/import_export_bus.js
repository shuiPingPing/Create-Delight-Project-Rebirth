Ponder.registry((event) => {
  event
    .create(['ae2:import_bus', 'ae2:export_bus'])
    .scene(
      'createdelightcore:import_export_bus',
      '输入总线与输出总线的使用',
      'createdelightcore:ponder_import_export_bus',
      (builder, util) => {
        ...
        scene.idle(40);
        scene
          .text(60, '输出总线可将网络中的物品输入到目标容器中', [1.5, 1.5, 3.5])
          .attachKeyFrame();
        ...
        scene.rotateCameraY(90);
        scene.idle(40);
        item1 = scene.world.createItemEntity([2.5, 5, 2.5], [0, 0, 0], 'ae2:certus_quartz_crystal');
        scene.idle(10);
        scene.world.modifyEntity(item1, (e) => {
          e.kill();
        });
    ...
    .scene(
      'createdelightcore:bus_transport',
      '使用输入/输出总线配合存储总线进行物流',
      'createdelightcore:ponder_bus_transport',
      ...
      (builder, util) => {
        ...
        scene.idle(20);
        item1 = scene.world.createItemEntity([1.5, 5, 3.5], [0, 0, 0], 'ae2:certus_quartz_crystal');
        scene.idle(10);
        scene.world.modifyEntity(item1, (e) => {
          e.kill();
        });
