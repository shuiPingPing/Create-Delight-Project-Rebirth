Ponder.registry((event) => {
  event
    .create([
      ...
      'createdelightcore:fission_reactor_controller',
    ])
    .scene(
      'createdelightcore:fission_reactor',
      '裂变反应堆的搭建',
      ...
      (builder, util) => {
        ...
        scene
          .text(
            40,
            '在这个多方块结构中, 所有的锻造钢机壳都可以被锻造钢输入/输出总线代替, 也可以替换成逻辑端口来进行控制',
            [2, 2.5, 6.5]
          )
          .attachKeyFrame();
        ...
        scene.world.replaceBlocks(
          [5, 3, 2, 7, 4, 2],
          Block.id('create_new_age:reactor_casing').blockState,
          true
        );
        scene.world.replaceBlocks(
          [2, 3, 5, 2, 4, 7],
        ...
        scene.world.replaceBlocks(
          [5, 3, 2, 7, 4, 2],
          Block.id('create_new_age:reactor_glass').blockState,
          false
        );
        scene.world.replaceBlocks(
          [2, 3, 5, 2, 4, 7],
            ...
            for (let y = 3; y < 10; y++) {
              ...
              if (!excludedPos.some((pos) => pos[0] == x && pos[1] == y)) {
                scene.world.setBlocks(
                  [x, z, y],
                  Block.id('createdelightcore:fission_fuel_assembly').blockState,
                  true
                );
                scene.idle(1);
              }
        ...
        scene.idle(40);
        // scene.playSound("alexscavesup:nuclear_explosion", 1, 1)
        // scene.playSound("alexscavesup:nuclear_explosion_ringing", 1, 1)
        // scene.playSound("alexscavesup:nuclear_explosion_rumble", 1, 1)
        // scene.world.createEntity(level => {
        //     return level.playLocalSound([6, 3, 6], "alexscavesup:nuclear_explosion", "ambient", 1, 1, true)
        // })
        ...
        scene.world.replaceBlocks(
          [0, 1, 0, 13, 10, 13],
          Block.id('minecraft:air').blockState,
          true
        );
        scene.idleSeconds(6);
